...
fun main(args: Array<String>) {
    ...
    driver(DriverParameters(startNodesInProcess = true, waitForAllNodesToFinish = true)) {
        listOf(
            startNode(providedName = CordaX500Name("ISW", "Victoria Island", "NG"), rpcUsers = rpcUsers),
            startNode(providedName = CordaX500Name("GatewayA", "Lagos", "NG"), rpcUsers = rpcUsers),
            startNode(providedName = CordaX500Name("GatewayB", "Port Harcourt", "NG"), rpcUsers = rpcUsers)
        ).map { it.getOrThrow() }
    }
}